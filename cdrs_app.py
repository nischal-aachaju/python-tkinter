...
# =============================================================
#   DATABASE SETUP
#   All tables are created here at startup — simple and clear
# =============================================================

# ── Auth database (login / register / forgot password) ──
...
# ── Main app database (doubts, participants, volunteers, sessions, rooms) ──
...
#   Simple functions — each does one job only
# =============================================================
...
def post_doubt(student_name, title, description):
    app_cur.execute(
        "INSERT INTO doubts (student_name, title, description) VALUES (?, ?, ?)",
...
def join_doubt(doubt_id, student_name):
    try:
        app_cur.execute(
            "INSERT INTO participants (doubt_id, student_name) VALUES (?, ?)",
...
def get_participants(doubt_id):
    app_cur.execute(
        "SELECT student_name, joined_at FROM participants WHERE doubt_id = ?",
        (doubt_id,)
    )
    return app_cur.fetchall()
...
def volunteer_for_doubt(doubt_id, volunteer_name):
    try:
        app_cur.execute(
            "INSERT INTO volunteers (doubt_id, volunteer_name) VALUES (?, ?)",
            (doubt_id, volunteer_name)
        )
        app_conn.commit()
        return True
    except sqlite3.IntegrityError:
        return False  # Already volunteered

def get_volunteers(doubt_id):
    app_cur.execute(
        "SELECT volunteer_name, volunteered_at FROM volunteers WHERE doubt_id = ?",
        (doubt_id,)
    )
    return app_cur.fetchall()
...
def schedule_session(doubt_id, teacher_name, room, scheduled_at):
    try:
        app_cur.execute(
            "INSERT INTO sessions (doubt_id, teacher_name, room, scheduled_at) VALUES (?, ?, ?, ?)",
        ...
        return False  # Session already exists for this doubt
...
def login_page():
    ...
    Button(register_frame, text="Register", width=27, bg="#00bcd4",
           fg="white", font=("Arial", 16, "bold"), command=register).place(x=50, y=380)
    ...
    Button(forgot_frame, text="Submit", width=27, bg="#00bcd4",
           fg="white", font=("Arial", 16, "bold"), command=reset_password).place(x=50, y=380)
...
def student_content(parent_frame, current_user, parent_root, doubt):
    # doubt = (id, student_name, title, description, status, posted_at)
    ...
    # Volunteer button — opens joining page
    ...
    Volunteer.bind("<Button-1>", lambda e: joining_page(parent_root, current_user, doubt_id, "volunteer"))

    # Get session info to show teacher name if scheduled
    ...
    Teacher.bind("<Button-1>", lambda e: joining_page(parent_root, current_user, doubt_id, "join"))
    ...
    # Combine participants and volunteers into one list
    ...
    if all_enrolled:
        for i, (pname, role) in enumerate(all_enrolled[:4]):
            ...
            Label(frame2, text=display, font=("Arial", 10), bg="#E1E9E5").place(x=30, y=30 + i * 20)
    else:
        Label(frame2, text="No students yet", font=("Arial", 10), fg="gray", bg="#E1E9E5").place(x=30, y=30)
    ...
    Enroll.bind("<Button-1>", lambda e: joining_page(parent_root, current_user, doubt_id, "join"))


def teacher_content(parent_frame, current_user, parent_root, doubt):
    # doubt = (id, student_name, title, description, status, posted_at)
    doubt_id, posted_by, title, description, status, posted_at = doubt
    ...
    Volunteer = Label(frame, text=vol_label, bd=2, relief="groove",
                      fg="white", bg="#23cff2", cursor="hand2", font=("Arial", 12, "bold"), padx=5, pady=2)
    Volunteer.place(x=30, y=110)
    Volunteer.bind("<Button-1>", lambda e: joining_page(parent_root, current_user, doubt_id, "teacher"))
    ...
    # Teacher joins as tutor — schedules a session
    Enroll = Label(frame2, text="Join as tutor", fg="white", bg="#23cff2", cursor="hand2",
                   font=("Arial", 12, "bold"), padx=5, pady=2, bd=2, relief="groove")
    Enroll.place(x=30, y=115)
    Enroll.bind("<Button-1>", lambda e: joining_page(parent_root, current_user, doubt_id, "teacher"))
...
def student_page(name):
    ...
    # Scrollable area for doubt cards
    ...
    # Load all doubts and render a card for each
    doubts = get_all_doubts()
    if doubts:
        for doubt in doubts:
            student_content(data_frame, name, student_root, doubt)
    else:
        Label(data_frame, text="No doubts posted yet.", font=("Arial", 14), bg="#f2f2f2", fg="gray").pack(pady=40)
...
def teacher_page(name):
    ...
    if doubts:
        for doubt in doubts:
            teacher_content(data_frame, name, teacher_root, doubt)
    else:
        Label(data_frame, text="No doubts posted yet.", font=("Arial", 14), bg="#f2f2f2", fg="gray").pack(pady=40)
...
def joining_page(prev_root, name, doubt_id, mode="join"):
    # mode = "join" (student joins), "volunteer" (student volunteers), "teacher" (teacher schedules)
    ...
    # Load this doubt's data from DB
    ...
    title_entry.insert(0, doubt[2])                   # real title from DB
    ...
    desc_text.insert("1.0", doubt[3])                 # real description from DB
    ...
    # Load enrolled participants from DB
    ...
    def refresh_enrolled():
        for widget in enrolled_frame.winfo_children():
            widget.destroy()
        Label(enrolled_frame, text="Enrolled", font=("Arial", 14), bg="#f2f2f2").pack(pady=10)
        ...
        if all_enrolled:
            for pname, role in all_enrolled:
                display = f"•  {pname} (volunteer)" if role == "volunteer" else f"•  {pname}"
                Label(enrolled_frame, text=display, font=("Arial", 11), bg="#f2f2f2", anchor="w").pack(fill="x", padx=20, pady=2)
        else:
            Label(enrolled_frame, text="No students yet", font=("Arial", 10), bg="#f2f2f2", fg="gray").pack(pady=5)

    refresh_enrolled()  # initial load
    ...
    # Action on button click — depends on mode
    def on_join_now():
        if mode == "join":
            success = join_doubt(doubt_id, name)
            if success:
                messagebox.showinfo("Success", "You have successfully joined the session!", parent=join_root)
                refresh_enrolled()
            else:
    ...
    join_btn = Button(main_frame, text=btn_text, font=("Arial", 14, "bold"),
                      bg="#23cff2", fg="white", width=15, bd=0, cursor="hand2",
                      activebackground="#1eb6d4", command=on_join_now)
